jQuery( function( $ )
{
    var template = $( '#tmpl-video-central-metaboxes-file-advanced' ).html();

    $( 'body' ).on( 'click', '.video-central-metaboxes-file-advanced-upload', function( e )
    {
        e.preventDefault();

        var $uploadButton = $( this ),
            $fileList = $uploadButton.siblings( '.video-central-metaboxes-uploaded' ),
            maxFileUploads = $fileList.data( 'max_file_uploads' ),
            mimeType = $fileList.data( 'mime_type' ),
            msg = maxFileUploads > 1 ? VideoCentralMetaboxesFile.maxFileUploadsPlural : VideoCentralMetaboxesFile.maxFileUploadsSingle,
            frame,
            frameOptions = {
                className: 'media-frame video-central-metaboxes-file-frame',
                multiple: true,
                title: VideoCentralMetaboxesFileAdvanced.frameTitle
            };
        ...
        frame.on( 'select', function()
        {
            ...
            if ( ids.length > 0 )
            {
                ...
                var data = {
                    action: 'video_central_metaboxes_attach_file',
                    post_id: $( '#post_ID' ).val(),
                ...
                $.post( ajaxurl, data, function( r )
                {
                    if ( r.success )
                    {
                        $fileList
                            ...
                            .trigger( 'update.VideoCentralMetaboxesFile' );
                    }
                }, 'json' );
            }
        } );
