(function ( $ ) {
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.canvas = this.$container.find( '.video-central-metaboxes-map-canvas' )[0];
			this.$coordinate = this.$container.find( '.video-central-metaboxes-map-coordinate' );
			this.$findButton = this.$container.find( '.video-central-metaboxes-map-goto-address-button' );
		...
		addListeners: function () {
			 ...
			 *      ('resize' Event)
			 */
			$( window ).on( 'video_central_metaboxes_map_refresh', function () {
				that.refresh();
			} );
		...
		autocomplete: function () {
			...
			if ( $( '.video-central-metaboxes-geo-binding' ).length ) {
	...
	$( function () {
		$( '.video-central-metaboxes-map-field' ).each( function () {
		...
		$( '.video-central-metaboxes-input' ).on( 'clone', function () {
			$( '.video-central-metaboxes-map-field' ).each( function () {
				var field = new MapField( $( this ) );
				field.init();
