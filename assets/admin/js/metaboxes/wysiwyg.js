...
jQuery( function ( $ ) {
	...
	function getOriginalId( $el ) {
		var $clones = $el.closest( '.video-central-metaboxes-clone' ).siblings( '.video-central-metaboxes-clone' ),
			id = '';
		$clones.each( function () {
			var currentId = $( this ).find( '.video-central-metaboxes-wysiwyg' ).attr( 'id' );
	...
	$( ':input.video-central-metaboxes-wysiwyg' ).each( update );
	$( '.video-central-metaboxes-input' ).on( 'clone', ':input.video-central-metaboxes-wysiwyg', update );
} );
