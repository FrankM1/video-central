jQuery( function ( $ ) {
	...
	var select = window.rwmbSelect = {
		...
		selectAllNone: function ( $input ) {
			...
			$input.on( 'click', '.video-central-metaboxes-select-all-none a', function ( e ) {
				e.preventDefault();
		...
		bindEvents: function ( $el ) {
			var $input = $el.closest( '.video-central-metaboxes-input' ),
				$clone = $input.find( '.video-central-metaboxes-clone' );
	...
	$( ':input.video-central-metaboxes-select' ).each( update );
	$( '.video-central-metaboxes-input' ).on( 'clone', ':input.video-central-metaboxes-select', update );
} );
