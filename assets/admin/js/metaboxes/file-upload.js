...
jQuery( function ( $ ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		className: 'video-central-metaboxes-upload-area',
		tagName: 'div',
		template: wp.template( 'video-central-metaboxes-upload-area' ),
		render: function () {
		...
		initialize: function ( options ) {
			...
			this.el.id = _.uniqueId( 'video-central-metaboxes-upload-area-' );
			this.render();

			//Areas
			this.dropzone = this.el;
			this.browser = this.$( '.video-central-metaboxes-browse-button' )[0];
		...
		initUploader: function () {
			...
			if ( extensions ) {
				this.plupload.filters.mime_types = [{title: i18nVcmMedia.select, extensions: extensions}];
			}
		...
		getExtensions: function () {
			...
			_.each( mimeTypes, function ( current, index ) {
				if ( i18nVcmMedia.extensions[current] ) {
					exts = exts.concat( i18nVcmMedia.extensions[current] );
				}
			} );
			return exts;
		}
