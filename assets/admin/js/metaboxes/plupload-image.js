jQuery( function ( $ )
{
	...
	$( '.video-central-metaboxes-images' ).on( 'click', '.video-central-metaboxes-delete-file', function ()
	{
		// Check if we need to show drop target
		var $dragndrop = $( this ).parents( '.video-central-metaboxes-images' ).siblings( '.video-central-metaboxes-drag-drop' );

		// After delete files, show the Drag & Drop section
		$dragndrop.removeClass( 'hidden' );
	} );

	$( '.video-central-metaboxes-drag-drop' ).each( function ()
	{
		// Declare vars
		var $dropArea = $( this ),
			$imageList = $dropArea.siblings( '.video-central-metaboxes-uploaded' ),
	...
	{
		$( 'li#' + file.id )
			.addClass( 'video-central-metaboxes-image-error' )
			.delay( 1600 )
	...
	{
		$ul.removeClass( 'hidden' ).append( '<li id="' + file.id + '"><div class="video-central-metaboxes-image-uploading-bar"></div><div id="' + file.id + '-throbber" class="video-central-metaboxes-image-uploading-status"></div></li>' );
	}
	...
	{
		$( '#' + file.id + '-throbber' ).html( '<img class="video-central-metaboxes-loader" height="64" width="64" src="' + RWMB.url + 'img/loader.gif">' );
	}
} );
