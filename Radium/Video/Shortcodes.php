...
class Radium_Video_Shortcodes {
    ...
    function player( $atts, $content=null ) {
        ...
        <video class="video-js vjs-default-skin{$class}" width="{$width}" height="{$height}" {$poster_attribute} {$controls_attribute} {$preload_attribute} {$autoplay_attribute} {$loop_attribute} {$muted_attribute} muted>
            {$mp4_source}
            {$webm_source}
            {$ogg_source}{$track}
        </video>
