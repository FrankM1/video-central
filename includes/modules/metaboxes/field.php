 ...
 */
abstract class Video_Central_Metaboxes_Field
{
    /**
     * Add actions
     *
    ...
    static function show( $field, $saved )
    {
        ...
        $meta        = Video_Central_Metaboxes_Core::filter( 'field_meta', $meta, $field, $saved );

        $begin = call_user_func( array( $field_class, 'begin_html' ), $meta, $field );
        $begin = Video_Central_Metaboxes_Core::filter( 'begin_html', $begin, $field, $meta );
        ...
        if ( $field['clone'] )
        {
            ...
            foreach ( $meta as $index => $sub_meta )
            {
                ...
                $class     = "video-central-metaboxes-clone video-central-metaboxes-{$field['type']}-clone";
                $sort_icon = '';
                if ( $field['sort_clone'] )
                {
                    $class .= ' video-central-metaboxes-sort-clone';
                    $sort_icon = "<a href='javascript:;' class='video-central-metaboxes-clone-icon'></a>";
                }
                $input_html = "<div class='$class'>" . $sort_icon;

                // Call separated methods for displaying each type of field
                $input_html .= call_user_func( array( $field_class, 'html' ), $sub_meta, $sub_field );
                $input_html = Video_Central_Metaboxes_Core::filter( 'html', $input_html, $sub_field, $sub_meta );
        ...
        else
        {
            ...
            $field_html = Video_Central_Metaboxes_Core::filter( 'html', $field_html, $field, $meta );
        }

        $end = call_user_func( array( $field_class, 'end_html' ), $meta, $field );
        $end = Video_Central_Metaboxes_Core::filter( 'end_html', $end, $field, $meta );

        $html = Video_Central_Metaboxes_Core::filter( 'wrapper_html', "$begin$field_html$end", $field, $meta );

        // Display label and input in DIV and allow user-defined classes to be appended
        $classes = "video-central-metaboxes-field video-central-metaboxes-{$field['type']}-wrapper " . $field['class'] ;
        ...
        $outer_html = Video_Central_Metaboxes_Core::filter( 'outer_html', $outer_html, $field, $meta );
    ...
    static function begin_html( $meta, $field )
    {
        ...
        {
            $field_label = sprintf(
                '<div class="video-central-metaboxes-label"><label for="%s">%s</label></div>',
                $field['id'],
                $field['name']
            );
        }
        ...
        $input_open = sprintf(
            '<div class="video-central-metaboxes-input"%s>',
            $data_max_clone
        );
    ...
    {
        $button = $field['clone'] ? call_user_func( array( Video_Central_Metabox::get_class_name( $field ), 'add_clone_button' ), $field ) : '';
    ...
    {
        $text = apply_filters( 'video_central_metaboxes_add_clone_button_text', __( '+ Add more', 'meta-box' ), $field );
        return "<a href='#' class='video-central-metaboxes-button button-primary add-clone'>$text</a>";
    }
    ...
    static function remove_clone_button( $field )
    {
        ...
        $text = apply_filters( 'video_central_metaboxes_remove_clone_button_text', $icon, $field );
        return "<a href='#' class='video-central-metaboxes-button remove-clone'>$text</a>";
    }
    ...
    static function meta( $post_id, $saved, $field )
    {
        ...
        // Escape attributes
        $meta = call_user_func( array( Video_Central_Metabox::get_class_name( $field ), 'esc_meta' ), $meta );
    ...
    static function get_attributes( $field, $value = null )
    {
        $attributes = wp_parse_args( $field['attributes'], array(
            ...
            'class'    => "video-central-metaboxes-{$field['type']}",
            'id'       => $field['id'],
    ...
    {
        $value  = call_user_func( array( Video_Central_Metabox::get_class_name( $field ), 'get_value' ), $field, $args, $post_id );
        $output = $value;
        if ( is_array( $value ) )
        {
            $output = '<ul>';
