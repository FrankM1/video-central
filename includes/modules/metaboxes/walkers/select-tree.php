 ...
 *
 * @uses Video_Central_Metaboxes_Walker_Select
 */
class Video_Central_Metaboxes_Walker_Select_Tree {
	...
	function display_level( $options, $parent_id = 0, $active = false ) {
		...
		$field      = $this->field;
		$walker     = new Video_Central_Metaboxes_Walker_Select( $this->db_fields, $field, $this->meta );
		$attributes = Video_Central_Metaboxes_Field::call( 'get_attributes', $field, $this->meta );

		$children = $options[ $parent_id ];
		$output   = sprintf(
			'<div class="video-central-metaboxes-select-tree %s" data-parent-id="%s"><select %s>',
			$active ? '' : 'hidden',
			$parent_id,
