...
class Video_Central_Metabox {
	...
	public function __construct( $meta_box ) {
		...
		$show = apply_filters( 'video_central_metaboxes_show', true, $this->meta_box );
		...
		foreach ( $this->fields as $field ) {
			Video_Central_Metaboxes_Field::call( $field, 'add_actions' );
		}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'video-central-metaboxes', Video_Central_Metaboxes_CSS_URL . 'style.css', array(), Video_Central_Metaboxes_VER );
		if ( is_rtl() ) {
			wp_enqueue_style( 'video-central-metaboxes-rtl', Video_Central_Metaboxes_CSS_URL . 'style-rtl.css', array(), Video_Central_Metaboxes_VER );
		}

		// Load clone script conditionally
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'video-central-metaboxes-clone', Video_Central_Metaboxes_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), Video_Central_Metaboxes_VER, true );
				break;
			}
		}
		...
		foreach ( $this->fields as $field ) {
			Video_Central_Metaboxes_Field::call( $field, 'admin_enqueue_scripts' );
		}

		// Auto save
		if ( $this->meta_box['autosave'] ) {
			wp_enqueue_script( 'video-central-metaboxes-autosave', Video_Central_Metaboxes_JS_URL . 'autosave.js', array( 'jquery' ), Video_Central_Metaboxes_VER, true );
		}
		 ...
		 */
		do_action( 'video_central_metaboxes_enqueue_scripts', $this );
	}
	...
	public function show() {
		...
		// 1st action applies to all meta boxes
		// 2nd action applies to only current meta box
		do_action( 'video_central_metaboxes_before', $this );
		do_action( "video_central_metaboxes_before_{$this->meta_box['id']}", $this );

		foreach ( $this->fields as $field ) {
			Video_Central_Metaboxes_Field::call( 'show', $field, $saved );
		}

		// Allow users to add custom code after meta box content
		// 1st action applies to all meta boxes
		// 2nd action applies to only current meta box
		do_action( 'video_central_metaboxes_after', $this );
		do_action( "video_central_metaboxes_after_{$this->meta_box['id']}", $this );
	...
	public function save_post( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			$old    = Video_Central_Metaboxes_Field::call( $field, 'raw_meta', $post_id );
			$new    = isset( $_POST[ $field['id'] ] ) ? $_POST[ $field['id'] ] : ( $single ? '' : array() );

			// Allow field class change the value
			if ( $field['clone'] ) {
				$new = Video_Central_Metaboxes_Clone::value( $new, $old, $post_id, $field );
			} else {
				$new = Video_Central_Metaboxes_Field::call( $field, 'value', $new, $old, $post_id );
				$new = Video_Central_Metaboxes_Field::filter( 'sanitize', $new, $field );
			}
			$new = Video_Central_Metaboxes_Field::filter( 'value', $new, $field, $old );

			// Call defined method to save meta value, if there's no methods, call common one
			Video_Central_Metaboxes_Field::call( $field, 'save', $new, $old, $post_id );
		}
	...
	protected function validate() {
		...
		return
			...
			&& wp_verify_nonce( $nonce, "video-central-metaboxes-save-{$this->meta_box['id']}" );
	}
	...
	public static function normalize_fields( $fields ) {
		foreach ( $fields as $k => $field ) {
			...
			$field = Video_Central_Metaboxes_Field::call('normalize', $field );

			// Allow to add default values for fields
			$field = apply_filters( 'video_central_metaboxes_normalize_field', $field );
			$field = apply_filters( "video_central_metaboxes_normalize_{$field['type']}_field", $field );
			$field = apply_filters( "video_central_metaboxes_normalize_{$field['id']}_field", $field );
	...
	public function is_saved() {
		...
		foreach ( $this->fields as $field ) {
			...
			$value = Video_Central_Metaboxes_Field::call( $field, 'raw_meta', $post->ID );
			if (
