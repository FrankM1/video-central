 ...
 * Map field class.
 */
class Video_Central_Metaboxes_Map_Field extends Video_Central_Metaboxes_Field {
	...
	static function admin_enqueue_scripts() {
		 ...
		 */
		$google_maps_url = apply_filters( 'video_central_metaboxes_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_style( 'video-central-metaboxes-map', Video_Central_Metaboxes_CSS_URL . 'map.css' );
		wp_enqueue_script( 'video-central-metaboxes-map', Video_Central_Metaboxes_JS_URL . 'map.js', array( 'jquery-ui-autocomplete', 'google-maps' ), Video_Central_Metaboxes_VER, true );
	}
	...
	static function html( $meta, $field ) {
		$html = '<div class="video-central-metaboxes-map-field">';

		$html .= sprintf(
			'<div class="video-central-metaboxes-map-canvas" data-default-loc="%s"></div>
			<input type="hidden" name="%s" class="video-central-metaboxes-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	static function the_value( $field, $args = array(), $post_id = null ) {
		...
		/*
		 * Enqueue scripts
		 * API key is get from $field (if found by Video_Central_Metaboxes_Helper::find_field()) or $args as a fallback
		...
		$google_maps_url = apply_filters( 'video_central_metaboxes_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), '', true );
		wp_enqueue_script( 'video-central-metaboxes-map-frontend', Video_Central_Metaboxes_JS_URL . 'map-frontend.js', array( 'google-maps' ), '', true );
		...
		$output = sprintf(
			'<div class="video-central-metaboxes-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
