 ...
 * @uses WP_Widget
 */
class Video_Central_Featured_Widget extends WP_Widget {

	/**
	 * video_central Featured Widget
	 *
	 * Registers the featured widget
	 *
	 * @since 1.0.0
	 *
	 * @uses apply_filters() Calls 'video_central_featured_widget_options' with the
	 *                        widget options
	 */
	public function __construct() {
		$widget_ops = apply_filters( 'video_central_featured_widget_options', array(
			'classname'   => 'widget_display_featured',
			'description' => __( 'The video central featured videos.', 'video_central' )
		) );

		parent::__construct( false, __( '(Video Central) Featured Videos', 'video_central' ), $widget_ops );
	}
	...
	/**
	 * Displays the output, the featured form
	 *
	 * @since 1.0.0
	 *
	 * @uses apply_filters() Calls 'video_central_featured_widget_title' with the title
	 * @uses get_template_part() To get the featured form
	 */
	public function widget( $args, $instance ) {
		...
		$settings['title'] = apply_filters( 'video_central_featured_widget_title', $settings['title'], $instance, $this->id_base );
				...
				<li class="clearfix">
					<div class="video-central-video-thumb">
						<a class="" href="<?php video_central_video_permalink( $widget_query->post->ID ); ?>">
                            ...
                            <?php do_action('video_central_before_featured_widget_thumb'); ?>

                            <img src="<?php video_central_featured_image_url( $widget_query->post->ID, array( 'width' => $thumb_w, 'height' => $thumb_h ) ); ?>" alt=""/>

                            <?php do_action('video_central_after_featured_widget_thumb'); ?>
	...
	/**
	 * Output the featured widget options form
	 *
	 * @since v1.0.0
	 *
	 * @param $instance Instance
	 * @uses Video_Central_Featured_Widget::get_field_id() To output the field id
	 * @uses Video_Central_Featured_Widget::get_field_name() To output the field name
	 */
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'video_central' ); ?>
				<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $settings['title'] ); ?>" />
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of videos to display:', 'video_central' ); ?>
				<input class="widefat" id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo esc_attr( $settings['number'] ); ?>" />
			</label>
		</p>
	...
	public function parse_settings( $instance = array() ) {
		return video_central_parse_args( $instance, array(
			'title' => __( 'Featured Videos', 'video_central' ),
			'number' => 5
		), 'featured_widget_settings' );
	}
