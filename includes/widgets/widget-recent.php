 ...
 * @uses WP_Widget
 */
class Video_Central_Recent_Widget extends WP_Widget {

	/**
	 * video_central Recent Widget
	 *
	 * Registers the recent widget
	 *
	 * @since 1.0.0
	 *
	 * @uses apply_filters() Calls 'video_central_recent_widget_options' with the
	 *                        widget options
	 */
	public function __construct() {
		$widget_ops = apply_filters( 'video_central_recent_widget_options', array(
			'classname'   => 'widget_display_recent',
			'description' => __( 'The video central recent videos.', 'video_central' )
		) );

		parent::__construct( false, __( '(Video Central) Recent Videos', 'video_central' ), $widget_ops );
	}
	...
	/**
	 * Displays the output, the recent form
	 *
	 * @since 1.0.0
	 *
	 * @uses apply_filters() Calls 'video_central_recent_widget_title' with the title
	 * @uses get_template_part() To get the recent form
	 */
	public function widget( $args, $instance ) {
		...
		$settings['title'] = apply_filters( 'video_central_recent_widget_title', $settings['title'], $instance, $this->id_base );
	...
	/**
	 * Output the recent widget options form
	 *
	 * @since v1.0.0
	 *
	 * @param $instance Instance
	 * @uses Video_Central_Recent_Widget::get_field_id() To output the field id
	 * @uses Video_Central_Recent_Widget::get_field_name() To output the field name
	 */
	...
	public function parse_settings( $instance = array() ) {
		return video_central_parse_args( $instance, array(
			'title' => __( 'Recent Videos', 'video_central' ),
			'number' => 5
		), 'recent_widget_settings' );
	}
}
