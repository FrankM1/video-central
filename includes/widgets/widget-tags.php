...
class Video_Central_Widget_Tags extends WP_Widget {
	...
	<p><label for="<?php echo $this->get_field_id('taxonomy'); ?>"><?php _e('Taxonomy:', 'video_central') ?></label>
	<select class="widefat" id="<?php echo $this->get_field_id('taxonomy'); ?>" name="<?php echo $this->get_field_name('taxonomy'); ?>">
	<?php foreach ( $taxonomy_objects as $tax ) : ?>
		<option value="<?php echo esc_attr($tax->name) ?>" <?php selected($tax->name, $current_taxonomy) ?>><?php echo $tax->label; ?></option>
	<?php endforeach; ?>
	</select></p><?php
	}
