...
function video_central_featured_image_url( $video_id = 0, $image_size = array() ) {
    ...
    function video_central_get_featured_image_url( $video_id = 0, $image_size = array() ){
        ...
        $placeholder_url = apply_filters( __FUNCTION__, video_central()->theme_compat->theme->url . 'img/placeholder.gif' );

        if ( !$image ) //Defines a default image
            $image = $placeholder_url;

        return apply_filters( __FUNCTION__, $image, $video_id, $image_size );
...
function video_central_has_related_videos( $args = '' ) {
	...
	if ( !video_central_allow_related_videos () ) return;
